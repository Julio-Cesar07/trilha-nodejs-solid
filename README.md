...
PS2... Caso não tenha o NodeJs instalado, o docker-compose.yml possui a imagem do node comentado, então caso necessário, remove o comentário, altere o .env como explicado no arquivo de exemplo e execute as imagens do node e postgres com ```sh docker compose up -d``` (será necessário ter o docker instalado). Para escrever um comando diferente na aplicação dentro do docker, execute no cmd na pasta raiz ```sh docker container exec nodejs sh -c '{comando_desejado}'```
